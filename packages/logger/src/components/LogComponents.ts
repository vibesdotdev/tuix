...
export function LogMessage({
  ...
  if (showTimestamp && timestamp) {
    parts.push(
      styledText(timestamp.toISOString(), style().foreground(color.gray))
    );
  }
  ...
  if (showContext && context && context.length > 0) {
    parts.push(
      styledText(`[${context.join(".")}]`, style().foreground(color.blue))
    );
  }
  ...
  return hstack(
    ...parts.map((part, i) => (i === 0 ? part : hstack(text(" "), part)))
  );
}
...
export function LogError({
  ...
  const parts: View[] = [
    styledText(
      `${indentStr}Error: ${error.message}`,
      style().foreground(color.red).bold()
    ),
  ];
  ...
  if (showStack && error.stack) {
    ...
    stackLines.forEach((line) => {
      parts.push(
        styledText(indentStr + line.trim(), style().foreground(color.gray))
      );
    });
  }
  ...
  return vstack(
    ...Object.entries(metadata).map(([key, value]) =>
      hstack(
        styledText(
          " ".repeat(indent) + key + ":",
          style().foreground(color.blue).bold()
        ),
        text(" "),
...
export const logUtils = {
  ...
  trace: (message: string) =>
    process.stdout.write(
      logUtils.formatLogLine("trace", message, { showEmoji: true }) + "\n"
    ),
  debug: (message: string) =>
    process.stdout.write(
      logUtils.formatLogLine("debug", message, { showEmoji: true }) + "\n"
    ),
  info: (message: string) =>
    process.stdout.write(
      logUtils.formatLogLine("info", message, { showEmoji: true }) + "\n"
    ),
  warn: (message: string) =>
    process.stderr.write(
      logUtils.formatLogLine("warn", message, { showEmoji: true }) + "\n"
    ),
  error: (message: string) =>
    process.stderr.write(
      logUtils.formatLogLine("error", message, { showEmoji: true }) + "\n"
    ),
  fatal: (message: string) =>
    process.stderr.write(
      logUtils.formatLogLine("fatal", message, { showEmoji: true }) + "\n"
    ),
};
