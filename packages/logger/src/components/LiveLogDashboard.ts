...
export function LiveLogDashboard({
  ...
  if (selectedPreset && LOG_PRESETS[selectedPreset]) {
    filteredLogs = filteredLogs.filter((log) =>
      matchesFilter(log, LOG_PRESETS[selectedPreset])
    );
  }
...
function createHeader(
  ...
  return hstack(
    ...
    text("  "),
    styledText(
      `${filteredLogs}/${totalLogs} logs${filterText}`,
...
function createFilterControls(preset: string, search: string): View {
  ...
  return vstack(
    ...
    hstack(
      styledText("Presets: ", style().foreground(color.gray)),
      text(presetList + "...")
    )
  );
}
...
function createProcessStats(
  ...
  const stats = processes.map((process) => {
    ...
    return hstack(
      ...
      text("  "),
      errorCount > 0
        ? styledText(`❌ ${errorCount}`, style().foreground(color.red))
        : text(""),
      text("  "),
      warnCount > 0
        ? styledText(`⚠️ ${warnCount}`, style().foreground(color.yellow))
        : text("")
    );
  });

  return [
    styledText("Process Status:", style().foreground(color.cyan).bold()),
    ...stats,
    text(""),
  ];
}
...
export function ProcessLogView({
  ...
  const displayLogs = followMode
    ? filteredLogs.slice(-maxLines)
    : filteredLogs.slice(0, maxLines);

  const header = hstack(
    styledText(
      `📝 Logs: ${processName}`,
      style().bold().foreground(color.blue)
    ),
    text("  "),
    styledText(
